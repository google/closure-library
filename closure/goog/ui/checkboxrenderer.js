...
goog.requireType('goog.ui.Checkbox');
...
goog.ui.CheckboxRenderer.prototype.decorate = function(checkbox, element) {
  ...
  var Checkbox = goog.module.get('goog.ui.Checkbox');
  var checked = Checkbox.State.UNCHECKED;
  if (goog.array.contains(
          classes,
          this.getClassForCheckboxState(Checkbox.State.UNDETERMINED))) {
    checked = Checkbox.State.UNDETERMINED;
  } else if (goog.array.contains(
                 classes,
                 this.getClassForCheckboxState(Checkbox.State.CHECKED))) {
    checked = Checkbox.State.CHECKED;
  } else if (goog.array.contains(
                 classes,
                 this.getClassForCheckboxState(Checkbox.State.UNCHECKED))) {
    checked = Checkbox.State.UNCHECKED;
  }
...
goog.ui.CheckboxRenderer.prototype.setCheckboxState = function(element, state) {
  ...
  if (element) {
    ...
    goog.object.forEach(Checkbox.State, function(state) {
      'use strict';
...
goog.ui.CheckboxRenderer.prototype.ariaStateFromCheckState_ = function(state) {
  'use strict';
  var Checkbox = goog.module.get('goog.ui.Checkbox');
  if (state == Checkbox.State.UNDETERMINED) {
    return 'mixed';
  } else if (state == Checkbox.State.CHECKED) {
    return 'true';
  } else {
    return 'false';
  }
