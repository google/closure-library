 ...
 * @private
 */
goog.jsaction.jsprops.ATTRIBUTE_NAME_ = 'jsprops';
 ...
 * @private
 */
goog.jsaction.jsprops.PROPERTY_KEY_ = '__jsprops';
...
goog.jsaction.jsprops.updateAttribute = function(elem) {
  elem.setAttribute(
      goog.jsaction.jsprops.ATTRIBUTE_NAME_,
...
goog.jsaction.jsprops.getPropertiesObject_ = function(elem) {
  var props = elem[goog.jsaction.jsprops.PROPERTY_KEY_];
  if (!props) {
    // If the property isn't set, we parse the attribute (if present) and
    // remember it.
    var attrVal = elem.getAttribute(goog.jsaction.jsprops.ATTRIBUTE_NAME_);
    if (attrVal) {
      props = goog.json.unsafeParse(attrVal);
    } else {
      props = {};
    }
    elem[goog.jsaction.jsprops.PROPERTY_KEY_] = props;
  }
  return props;
};
