...
/**
 * @fileoverview Unit tests for goog.userAgent.
 */
...
function assertUserAgent(expectedAgents, uaString, opt_product, opt_vendor) {
  ...
  propertyReplacer.set(
      ...
      'getUserAgent',
      goog.functions.constant(uaString));
...
function testOperaInit() {
  propertyReplacer.set(goog, 'global', {
    'opera': {
      'version': goog.functions.constant('9.20')
    }
  });

  var uaString = 'Opera/9.20 (Windows NT 5.1; U; de),gzip(gfe)';
...
function testOpera() {
  var assertOpera = function(uaString) {
    assertUserAgent([goog.userAgentTestUtil.UserAgents.OPERA], uaString);
  };
  var assertIe = function(uaString) {
    assertUserAgent([goog.userAgentTestUtil.UserAgents.IE], uaString);
  };
...
function assertGecko(uaString, expectedVersion) {
  assertUserAgent([goog.userAgentTestUtil.UserAgents.GECKO], uaString, 'Gecko');
  assertEquals('User agent ' + uaString + ' should have had version ' +
      expectedVersion + ' but had ' + goog.userAgent.VERSION,
      expectedVersion,
      goog.userAgent.VERSION);
