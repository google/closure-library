...
/**
 * @fileoverview Tests for goog.userAgent.product.
 */
...
function setUserAgentString(ua) {
  replacer.set(goog.labs.userAgent.util, 'getUserAgent',
               function() { return ua; });
}
...
function assertIsBrowser(browser) {
  goog.array.forEach(DETECTED_BROWSER_KEYS, function(key) {
    var isBrowser = (key == browser);
    assertEquals('Expected value for browser key: ' + key,
                 isBrowser, goog.userAgent.product[key]);
  });
}
