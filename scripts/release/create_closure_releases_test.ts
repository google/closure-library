...
import {
  clientImplementationsForTesting,
  createClosureReleases,
} from "./create_closure_releases";
 ...
 * @param fakeRecentDrafts An array of recent drafts to return when GitHubClient#getRecentDrafts is called.
 */
function spyOnClients(
  fakeLatestReleaseHash: string,
  fakeGitCommits: FakeGitData[],
  ...
  const gitCommits: Change[] = fakeGitCommits.map((commit) =>
    Object.assign({ body: "" }, commit)
  );
  // Make GitClient#listCommits list commits in `fakeGitCommits`.
  spyOn(GitClient.prototype, "listCommits").and.callFake(
    async ({ from, to }) => {
  ...
  // `fakeGitCommits`.
  spyOn(GitClient.prototype, "getFile").and.callFake(
    async (commitish, file) => {
      assert.strictEqual(file, "package.json");
      const { pJsonVersion } = fakeGitCommits.find(
        (commit) => commit.hash === commitish
      );
  ...
  spyOn(GitHubClient.prototype, "getLatestReleaseTag").and.returnValue(
    Promise.resolve(fakeLatestReleaseHash)
  );
  // Make GitHubClient#getLatestRelease return `FAKE_RELEASE_URL`.
  spyOn(GitHubClient.prototype, "draftRelease").and.returnValue(
    Promise.resolve(FAKE_RELEASE_URL)
  );
  // Make GitHubClient#getLatestRelease return `fakeRecentDrafts`.
  spyOn(GitHubClient.prototype, "getRecentDrafts").and.returnValue(
    Promise.resolve(fakeRecentDrafts)
  );
}
...
describe("createClosureReleases", () => {
  ...
  it("constructs exactly one instance of each client class", async () => {
    ...
    expect(clientImplementationsForTesting.GitHubClient).toHaveBeenCalledTimes(
      1
    );
  });

  it("passes a Closure-library specific args and release token to GitHubClient", async () => {
    spyOnClients("00", [
      { hash: "00", pJsonVersion: "20201009.0.0", message: "" },
    ]);
    spyOn(clientImplementationsForTesting, "GitHubClient").and.callThrough();
    await createClosureReleases(FAKE_TOKEN);
    expect(
      clientImplementationsForTesting.GitHubClient
  ...
  it(`doesn't create a release when there are no new commits found`, async () => {
    spyOnClients("00", [
      { hash: "00", pJsonVersion: "20201009.0.0", message: "" },
    ]);
    await createClosureReleases(FAKE_TOKEN);
  ...
  it("doesn't create a release when there are no version bumps found", async () => {
    spyOnClients("00", [
      { hash: "00", pJsonVersion: "20201009.0.0", message: "" },
  ...
  it("places multiple commits under the same release notes section", async () => {
    spyOnClients("00", [
      { hash: "00", pJsonVersion: "20201009.0.0", message: "" },
      {
        hash: "01",
  ...
  it("de-duplicates release notes with the same text and change type", async () => {
    spyOnClients("00", [
      { hash: "00", pJsonVersion: "20201009.0.0", message: "" },
      {
        hash: "01",
  ...
  it('writes a release notes entry for invalid rollbacks under "Other Changes"', async () => {
    spyOnClients("00", [
      { hash: "00", pJsonVersion: "20201009.0.0", message: "" },
      {
        hash: "01",
  ...
  it("updates a draft rather than create a new one with the same tag", async () => {
    spyOnClients(
      "00",
      [
        { hash: "00", pJsonVersion: "20201009.0.0", message: "" },
  ...
  it("creates a new draft rather than update one with a different tag", async () => {
    spyOnClients(
      "00",
      [
        { hash: "00", pJsonVersion: "20201009.0.0", message: "" },
